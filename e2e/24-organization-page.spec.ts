...
test.describe('Organization 페이지', () => {
    ...
    test('사용자 Drag & Drop 재배정 (기능 존재 시)', async ({ page }) => {
        ...
        if (!await userList.isVisible().catch(() => false)) {
            ...
            await expect(page.locator('.cursor-grab').first()).toBeVisible({ timeout: 5000 }).catch(() => { })
        }
        ...
        try {
            ...
            // 확인 버튼 클릭
            const confirmButton = confirmDialog.locator('button').filter({ hasText: /확인|이동|Confirm|Move/i }).first()
