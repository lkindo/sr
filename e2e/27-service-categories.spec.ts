...
test.describe('서비스 카테고리', () => {
    ...
    test('SR 생성 시 서비스 카테고리 선택', async ({ page }) => {
        ...
        // SR 생성 버튼 클릭
        const createButton = page.locator('button').filter({ hasText: /SR 요청|등록|생성|New SR/i }).first()
        ...
        // 서비스 카테고리 Select 찾기
        const categorySelect = dialog.locator('select[name*="category"], [role="combobox"]').filter({ hasText: /카테고리|Category|서비스/i }).first()
        ...
        if (linkVisible) {
            ...
            if (isClientDetailPage) {
                ...
                if (clientCategoriesResponse) {
                    ...
                    const categories = Array.isArray(clientCategoriesResponse) ? clientCategoriesResponse : (clientCategoriesResponse.data || [])
    ...
    test('서비스 카테고리 필터링', async ({ page }) => {
        ...
        // 카테고리 필터 Select 찾기
        const categoryFilter = page.locator('select[name*="category"], [role="combobox"]').filter({ hasText: /카테고리|Category/i }).first()
