...
test.describe('사용자 프로필 관리', () => {
  ...
  test('프로필 섹션 확인', async ({ page }) => {
    ...
    // 프로필 관련 섹션 또는 콘텐츠 찾기
    const profileSection = page.locator('section, div, form').filter({ hasText: /프로필|Profile|이름|Name|이메일|Email/i })
  ...
  test('프로필 수정 기능 확인', async ({ page }) => {
    ...
    // 수정 버튼 또는 입력 필드 찾기
    const editButton = page.locator('button').filter({ hasText: /수정|Edit|변경|저장|Save/i }).first()
    const inputField = page.locator('input[name*="name"], input[name*="email"], input[type="text"]').first()
  ...
  test('비밀번호 변경 섹션 확인', async ({ page }) => {
    ...
    // 비밀번호 관련 섹션 찾기
    const passwordSection = page.locator('section, div, form').filter({ hasText: /비밀번호|Password/i })
