...
- **성능 로깅**: 처리 소요 시간 계측은 **개별 라우트가 아니라 공용 래퍼**(`src/lib/auth-wrapper.ts`의 `withAuth`/`withErrorHandler`)에서 일괄 수행하며, 래퍼는 응답 직후 `logger.logRequest(method, path, status, duration)`를 호출한다. **모든 API 라우트는 이 래퍼를 경유한다.** 예외는 구조상 감쌀 수 없거나 의도적으로 비인증인 라우트(NextAuth 핸들러, SSE 스트림, 헬스체크, VAPID 공개 키 등)로 한정하며, 목록과 이유는 `docs/LLD.md` 'API 엔드포인트 카탈로그'의 예외 표 한 곳에 둔다. 프로덕션 로그 잡음을 줄이기 위해 임계값(기본 500ms)을 넘는 요청만 `warn` 레벨로 승격한다.
  <sub>정정(2026-09-18): 이 조항은 "API 라우트와 Server Action 모두 예외 없이 경유" 였지만, 래퍼 시그니처가 `(NextRequest, ctx)` 라 **Server Action 은 구조상 감쌀 수 없고** 실제로 하나도 경유하지 않는다. Server Action 의 인증·인가는 `src/lib/action-helpers.ts` 가 맡는다. ⚠️ **남은 항목**: Server Action 의 소요 시간 계측은 아직 없다.</sub>
...
- **NextAuth 세션 검증**: 사용자 요청을 처리하는 모든 백엔드 엔드포인트는 호출 즉시 NextAuth의 세션 정보를 조회하여 유효한 토큰 및 로그인 세션을 가졌는지 검증해야 한다. 예외는 로그인 전에 쓰여야 해서 **의도적으로 비인증인** 엔드포인트(NextAuth 핸들러, 헬스체크, VAPID 공개 키, 회원가입 화면의 공개 고객사 목록, 회원가입 액션, 가입 이메일 확인 링크)뿐이며, 이들은 개인정보·SR 데이터를 돌려주지 않는다. 이메일 확인 링크(`GET /api/register/verify-email`)는 서명된 토큰만 받아 결과를 로그인 화면으로 돌려보내고, IP 당 strict 로 제한한다(2026-09-18 소유자 결정 D13 B+). 공개 고객사 목록은 활성 고객사의 이름·코드만 주고 내부 id 는 주지 않으며, IP 당 strict 로 제한한다(`GEMINI.md` §1.2 예외, 2026-09-18 소유자 결정 D14 A+). 목록은 `docs/LLD.md` 'API 엔드포인트 카탈로그'에 둔다.
- **세분화된 권한 검사 (RBAC)**: 세션 검증을 통과한 후, 사용자의 역할이 해당 액션을 수행할 자격이 있는지 확인하는 인가 로직을 반드시 비즈니스 코드 최상단에 추가한다. **역할 정의의 정본은 `GEMINI.md` §1.1과 `prisma/seed.ts`의 역할 시드이며, 본 문서는 역할 이름을 복제하지 않는다.** 시드의 역할별 권한 매핑은 ADMIN 이 운영 중 조정하는 기본값이고, 기본 역할의 이름은 바꾸거나 지울 수 없다(`GEMINI.md` §1.4). 그래서 인가 코드는 "기본 권한이 없으니 불가능" 에 기대지 말고, 권한을 받아도 지켜야 할 신원·범위 조건(배정 격리, 테넌트 격리)을 함께 검사한다.
...
- **비동기 처리**: 요청 응답이 발송 결과를 기다려서는 안 된다. 이메일은 아래 아웃박스에 적재만 하고 발송은 디스패처(`src/instrumentation.ts` 가 기동)가 맡는다. 웹 푸시는 `src/lib/wait-until.ts`의 `backgroundTask` 헬퍼로 비차단 실행한다.
...
- **실패를 삼키지 않는다(이메일)**: 이메일 발송 함수는 실패 시 예외를 던진다. `Promise.allSettled`로 감싸 항상 fulfilled로 만들면 실패 경로가 코드상 존재하지 않는 것과 같아지고, 디스패처가 `failReason`을 기록할 수 없다.
  웹 푸시는 위 '웹 푸시의 예외'에 따라 재시도가 없는 채널이라, 여러 수신자에게 보내는 묶음을 `Promise.allSettled` 로 모아 한 수신자의 실패가 다른 수신자를 막지 않게 한다. 대신 구독별 실패는 발송 함수 안에서 로그로 남긴다(`push.service.ts`).
