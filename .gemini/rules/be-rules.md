...
- **NextAuth 세션 검증**: 사용자 요청을 처리하는 모든 백엔드 엔드포인트는 호출 즉시 NextAuth의 세션 정보를 조회하여 유효한 토큰 및 로그인 세션을 가졌는지 검증해야 한다. 예외는 로그인 전에 쓰여야 해서 **의도적으로 비인증인** 엔드포인트(헬스체크, VAPID 공개 키, 회원가입 화면의 공개 고객사 목록, 회원가입 액션)뿐이며, 이들은 개인정보·SR 데이터를 돌려주지 않는다. 목록은 `docs/LLD.md` 'API 엔드포인트 카탈로그'에 둔다.
