...
- **연쇄 삭제 주의**: Prisma 모델 정의 시 `onDelete: Cascade`는 신중하게 선택되어야 한다. **현재 저장소에서 실질적 위험은 `SR` 의 물리 삭제다** — SR 1건을 `DELETE` 하면 활동·댓글·첨부·상태이력 4개 테이블의 자식 행이 FK `onDelete: Cascade` 로 함께 영구 삭제된다. 앱의 SR 삭제는 논리 삭제(§2)라 이 경로를 타지 않으며, 앱에는 SR 을 물리 삭제하는 경로가 없다. 이 경고는 앞으로 만들 정리 배치·스크립트를 위한 것이다.
