...
> | 4.5 `/srs` 배지 `countSRs` 6회 | **처리 완료** — 단일 `COUNT(*) FILTER` 집계로 통합 (16dec3b, [#249](https://github.com/lkindo/sr/issues/249)) |
...
> 2026-08 정비(Phase 0-5)의 근거와 결정은 그 커밋 메시지에 남아 있다:
> `git log --grep='CLEANUP_PLAN\|Phase [0-5]'`.
>
...
- ~~**`tsconfig`에 `noUncheckedIndexedAccess` 없음**~~ — **해소(2026-08-02).** `noUncheckedIndexedAccess`, `noImplicitReturns`, `noFallthroughCasesInSwitch` 세 가지를 켰다. 켜자마자 164개 오류가 났고(프로덕션 34 / 테스트·E2E 130) 전부 정리했다. `exactOptionalPropertyTypes` 는 켜지 않았다 — 나머지 셋과 달리 선택 속성에 `undefined` 를 명시 대입하는 광범위한 패턴을 깨뜨려 이번 범위를 넘어선다.

  단순히 `!` 를 뿌리지 않았다. 반복되는 것은 원인을 없앴다:
  - `error.issues[0].message` 13곳 → `firstZodIssueMessage()` 하나로 통합(`src/lib/errors.ts`). 각 자리에 `?.` 를 붙이면 폴백 문구가 제각각이 되는데, 실제로 이미 일부는 폴백이 있고 일부는 없었다.
  - `SLA.PRIORITY_MULTIPLIER` 의 `as Record<string, number>` 제거. 키를 string 으로 넓혀 놓은 것이 인덱스 접근을 `| undefined` 로 만든 원인이었다. 키는 실제로 우선순위 4개뿐이고 호출부도 zod enum 으로 검증된 값을 넘긴다.
  - `sr.service.ts` 의 SR 번호 채번은 `sequences[0].seq` 를 그대로 읽고 있었다. `RETURNING` 이 빈 배열이면 `undefined.seq` 로 죽어 원인을 알 수 없는 500 이 되므로, 무슨 일이 있었는지 말해 주는 오류로 바꿨다.
  - `src/app/(dashboard)/srs/page.tsx` 의 `{ [sortField]: order }` 는 타입 오류이자 **4.1 이 API 경로에서 지적한 미검증 정렬 필드와 같은 결함**이었다(임의 컬럼이 그대로 Prisma orderBy 로 들어간다). allowlist 를 도입해 둘 다 닫았다.

  테스트 파일 130건은 병렬 작업으로 처리했고, 단언을 약화하거나 제거한 곳은 없다(오류 1건당 1줄 변경). 켠 뒤 `xs[0].toUpperCase()` 를 심어 게이트가 실제로 실패하는 것을 확인했다.

  원문: **`tsconfig`에 `noUncheckedIndexedAccess` 없음** — `tsconfig.json:2-24`가 `"strict": true`만 활성화한다. `noUncheckedIndexedAccess`, `noImplicitReturns`, `noFallthroughCasesInSwitch`, `exactOptionalPropertyTypes` 부재. 코드베이스가 인덱스 시그니처에 크게 의존한다(`src/lib/constants/sr.ts:6, 17, 28, 36`의 `Record<string, string>`, `env-validation.ts:273-281`의 `[category]` 조회). `statusLabels[sr.status]`가 맵에 없는 새 enum 값에 대해서도 `string`으로 타입되므로 **상태머신에 상태를 추가하고 `statusLabels` 갱신을 잊으면 타입체크를 통과하고 한국어 UI에 `undefined`가 렌더링**된다 — 생명주기 변경 시 이 앱이 정확히 맞을 실패 모드다. → `noUncheckedIndexedAccess: true` + `noFallthroughCasesInSwitch` + `noImplicitReturns` 활성화. 새 에러는 `!` 단언이 아니라 맵을 `Record<SRStatus, string>`(Prisma enum 키잉으로 exhaustive)으로 타이핑해 해결.
...
1. **"대시보드 stats 캐시 실제 연결"** — 실제로는 죽은 `baseCacheKey`/`cacheKey` 를 **제거**했고,
   ...
   표현만 정정했다.
2. **"`/srs` 5개 badge 카운트를 단일 `FILTER` 집계로"** — 감사 시점에는 `countSRs` 6회
   병렬 호출이었다. 2026-08-08 에 `SRService.getSRBadgeCounts` 로 통합했다(16dec3b) —
   카운트 쿼리 6회 → 2회. 테넌트 술어를 생 SQL 로 옮기는 부분이 위험 지점이라,
   교차 테넌트 유출 회귀 테스트를 실제 Postgres 로 함께 넣었다.
