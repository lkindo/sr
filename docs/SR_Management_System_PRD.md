...
- 이메일 발송 실패 시 자동 재시도, 사용자 정보 오류 시 관리자 알림 — **미구현**
  (현재 SMTP 실패는 로그만 남기고 재시도/영속 outbox 가 없다.
  `notifications` 테이블이 outbox 형태로 설계되어 있으나 아직 사용되지 않는다.)
...
- 영속 큐가 없다. 프로세스가 종료되면 진행 중인 발송은 유실된다.
- 발송 실패에 대한 재시도와 dead-letter 가 없다.
- `notifications` 테이블(PENDING/SENT/FAILED, `failReason`)이 outbox 로 설계되어 있으나
  아직 어떤 코드도 이 테이블에 기록하지 않는다.
...
| 미처리 경고    | SLA 초과 시작 | 담당자        | 이메일+푸시 | 1시간마다 |
...
- 이메일 발송: 도메인 이벤트 리스너에서 fire-and-forget (재시도/outbox 미구현)
...
- 웹 푸시(VAPID) 알림
- 알림 발송 기록 및 재시도 로직 — **미구현**
  (계획 당시의 Resend / React Email / Inngest / 매터모스트 Webhook 은 모두 채택되지 않았다)
