...
위 표는 **기본값**입니다. ADMIN은 운영 중 `/roles`에서 역할별 권한을 조정할 수 있으므로 최종 허용
여부는 역할명뿐 아니라 현재의 `RESOURCE:ACTION` 권한과 서버 정책으로 결정됩니다. 다만 권한을
조정해도 바뀌지 않는 규칙이 있습니다: 확인완료는 신청자(운영자가 등록한 SR은 등록자 또는 그
고객사 관리자)만 하고, `ENGINEER`는 권한을 받아도 배정된 SR만 다루며, 고객사 간 격리와 ADMIN
전용 기능(시스템 설정·알림 아웃박스·사용자 완전 삭제)은 그대로입니다. 외부 역할이 내부 역할을
부여하거나 자신의 고객사 밖 사용자를 관리하는 것은 차단됩니다.
...
- 비밀번호 변경·관리자 초기화가 성공하면 기존 세션 버전이 올라가므로 이전 세션은 더 이상
  유효하지 않습니다.
- 같은 계정으로 15분 안에 비밀번호를 10번 틀리면 그 계정은 15분 동안 로그인할 수 없습니다. 시간이
  지나면 자동으로 풀립니다. 짧은 시간에 너무 자주 시도해도 잠시 기다리라는 안내가 나옵니다.
- 로그인한 지 12시간이 지나면 사용 중이어도 다시 로그인해야 합니다.
...
- `CLIENT`는 고객사를 반드시 선택합니다. 목록은 로그인 없이 보이는 활성 고객사의 이름·코드이며,
  서버는 고른 코드로 활성 고객사를 다시 찾습니다(없거나 비활성이면 가입을 거부). 계정은 생성되지만
  고객사 소속은 `PENDING`으로 저장되며 승인 전에는 해당 고객사 데이터에 접근할 수 없습니다.
- `ENGINEER`는 비활성 상태로 생성되어 운영자 승인이 필요합니다.
- 가입하면 입력한 이메일로 확인 링크가 갑니다(24시간 유효). 링크를 열면 로그인 화면에 결과가 보입니다.
  확인하지 않아도 가입과 승인은 진행되지만, 승인 화면(사용자 관리·자사 사용자)에는 확인하지 않은 신청자에게
  **이메일 미인증** 표시가 붙습니다. 다른 사람의 이메일로 낸 신청일 수 있으니 본인 여부를 한 번 더 확인하세요.
- 휴대폰 본인 인증, 부서/팀 선택 기능은 구현되어 있지 않습니다.
...
- **지연 중**(내부 사용자 전용): 마감을 넘긴 진행 중(접수·진행중·보류) SR 건수입니다. 보류 건수와
  마감일을 직접 조정한 건수를 함께 보여 주며, 카드를 누르면 같은 SR만 걸러진 목록으로 이동합니다.
  SLA 준수율은 끝난 SR만 세므로 쌓이는 지연은 이 숫자로 봅니다. 아직 접수되지 않은 요청은
  여기에 잡히지 않으니 '평균 대기 시간'을 함께 봅니다.
- SLA 준수율 카드는 최근 30일에 끝난 SR이 없으면 '표본 없음', 마감일이 없어 판정할 수 없는 건은
  '측정 불가'로 표시합니다.
- `ENGINEER`의 '내 담당 SR'에는 끝나지 않은 SR만 마감일이 가까운 순서로 나옵니다.
...
- `/srs`: 권한 범위 안의 SR 전체 목록. 상태, 우선순위, 고객사, 담당자, 날짜와 검색어로
  필터링하고 정렬할 수 있습니다. 내부 사용자는 빠른 필터 **접수·담당·긴급·지연**을 씁니다('지연'은
  마감을 넘긴 진행 중 SR, 주소는 `/srs?overdue=1`). '오늘 마감' 배지는 지금부터 오늘 자정 전에
  마감이 오는 SR만 셉니다. 마감 열은 상태를 다시 적지 않습니다 — 끝난 SR(완료·확인완료·거절)은 '-' 이고,
  보류 SR도 다른 진행 중 SR처럼 'D-5'·'N일 지연'으로 보입니다.
- 배지 색: 상태는 요청됨 테두리·접수 회색·진행중 파랑·보류 노랑·완료 초록·확인완료 초록(체크)·거절 빨강,
  우선순위는 긴급 빨강·높음 주황, 마감은 지연·오늘·내일 빨강·D-2~3 주황입니다. 빨강은 거절·긴급·마감 임박에만 씁니다.
...
첨부는 폼 기준 최대 5개, 파일당 10MB입니다. 구형 Office 문서(`.doc`·`.xls`·`.ppt`)도 올릴 수
있습니다(2026-09-18 소유자 결정). 이 형식은 매크로를 담을 수 있으므로 내려받은 파일은 매크로를
켜지 말고 여세요. 파일은 웹 공개 디렉터리가 아닌 서버
...
- **SLA 마감일**은 접수 때 서비스 카테고리 SLA 시간 × 우선순위 배율로 자동 산출됩니다. 운영자는 SR
  ...
  다시 계산되지 않습니다. 사유는 감사 기록에 남고, 활동 이력에는 "SLA 마감일 조정: 이전 → 이후" 가 남습니다
  (사유는 내부 사용자에게만 보입니다).
- **한 번 완료된 SR**(재오픈된 SR 포함)은 처음 마감일을 유지합니다. 카테고리·우선순위를 바꿔도 다시
  계산되지 않고, 마감일 조정은 `ADMIN`·`MANAGER`만 할 수 있으며, 거절로 끝낼 수 없습니다(재작업이 끝나면
  다시 완료 처리). 완료·확인완료·거절된 SR 의 마감일·카테고리·실제 우선순위는 바꿀 수 없으니, 고쳐야 하면
  다시 연 뒤 수정합니다.
...
**운영 > 감사 로그**(`/settings/audit`, `ADMIN` 전용)에서 사용자·고객사·역할·카테고리 관리, 가입 승인,
마감일 조정, 삭제, 로그인 같은 기록을 행위·대상·행위자·기간으로 찾고 변경 내용을 펼쳐 볼 수 있습니다.
기록은 지우지 않습니다. 다른 사람에 대한 관리 이력(가입 승인 등)이 있는 계정은 영구 삭제되지 않고 비활성화로
남으며, `ADMIN`·`MANAGER` 계정은 삭제 버튼이 아니라 역할 변경이나 활성 토글로 다룹니다.

`/roles`는 기본적으로 `ADMIN`과 `ROLE:READ` 권한이 있는 사용자에게 제공됩니다. 역할 생성,
수정, 삭제와 권한 카탈로그 매핑을 관리합니다.

- 기본 역할 5개(`ADMIN`·`MANAGER`·`ENGINEER`·`CLIENT_ADMIN`·`CLIENT_USER`)는 ADMIN도 이름을
  바꾸거나 삭제할 수 없고, 새 역할에 그 이름(대소문자 무시)을 쓸 수도 없습니다. 시스템이 역할
  이름으로 권한을 판정하므로, 이름이 바뀌면 그 역할 사용자 전원이 권한을 잃기 때문입니다. 설명은
  고칠 수 있습니다.
- 기본 역할의 권한 구성은 ADMIN이 조정할 수 있는 기본값입니다. 예를 들어 `ENGINEER`에
  `SR:DELETE`를 주면 배정된 SR을 삭제할 수 있게 됩니다. 조정 내역은 감사 로그에 남고 재시작해도
  되돌아가지 않습니다.
- `ADMIN` 역할은 설명·권한을 포함해 아무도 수정할 수 없습니다(화면의 수정·권한 관리 버튼이 비활성).
- 행위자가 가진 권한보다 넓은 권한을 부여하거나 자기 역할을 수정하는 것은 금지됩니다(ADMIN 제외).
...
| SR 담당자 배정 | 새 담당자 |
| SR 상태 변경 | 신청자 — **완료·거절 메일은 설정과 무관하게 항상 발송**(필수 알림). 운영자가 자기 이름으로 등록한 SR은 그 운영자가 신청자이므로 고객사 관리자에게는 가지 않습니다. 본인이 한 상태 변경은 본인에게 알리지 않습니다 |
| SR 재오픈 | 담당자(재오픈 사유 포함, 'SR 배정' 설정을 따름). 담당자가 비활성 계정이면 활성 `ADMIN`·`MANAGER`에게 재배정 요청으로 갑니다 |
...
**계속 유지**를 누르면 연장됩니다. 이 타이머는 열려 있는 화면에서 돕니다. 창이나 탭을 닫기만 하면
로그아웃되지 않고, 마지막 사용 후 8시간 동안 로그인 상태가 남습니다. 공용 PC 에서는 반드시 **로그아웃**을
누르세요.
...
- 이메일 확인 링크 재발송(링크를 놓쳤으면 승인 담당자가 본인 여부를 직접 확인합니다)
- 휴대폰 본인 인증, 부서·팀 계층 관리, 자동 휴면 계정
- 2단계 인증(2FA), 비밀번호 정기 만료·재사용 금지, 관리자의 잠금 해제 화면
- 첨부 파일 바이러스 검사, 앱 수준 저장 암호화
- 매터모스트 연동 및 인앱 알림함
