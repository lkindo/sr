...
**알림 트리거:**
1. SR 신청 완료 → 담당자에게 즉시 알림
2. SR 접수됨 → 신청자에게 즉시 알림
3. SR 완료됨 → 신청자에게 즉시 알림
4. SR 보류됨 → 신청자 및 담당자에게 즉시 알림
5. SR 거절됨 → 신청자에게 즉시 알림
6. 장기 미처리 SR → 담당자에게 경고 (설정 가능한 시간 초과 시)
7. 담당자 배정됨 → 새 담당자에게 즉시 알림
8. 계약 종료 임박 → 고객사 관리자에게 알림 (30일, 14일, 1일전)
...
last_name
profile_image_url
status (ACTIVE, INACTIVE, SLEEPING, DELETED)
last_login_at
created_at (NOT NULL)
updated_at (NOT NULL)
created_by
...
```
id (PK)
client_code (UNIQUE, NOT NULL)
client_name (UNIQUE, NOT NULL)
industry_type
region
...
assigned_date
unassigned_date
is_primary (기본 고객사 여부)
created_at (NOT NULL)
updated_at (NOT NULL)
```

#### 4. ClientHandler (고객사 담당자)
```
id (PK)
client_id (FK)
user_id (FK)
mattermost_id
backup_handler_id (FK) (대체 담당자)
assigned_date
unassigned_date
created_at (NOT NULL)
updated_at (NOT NULL)
...
sla_hours (SLA 기한)
priority (우선순위)
handler_id (FK) (기본 담당자)
backup_handler_id (FK)
is_active (NOT NULL)
created_at (NOT NULL)
updated_at (NOT NULL)
```

#### 6. SR (Service Request)
```
id (PK)
sr_code (UNIQUE, NOT NULL) [예: ABC-20251106-001]
client_id (FK) (NOT NULL)
requester_id (FK) (NOT NULL)
handler_id (FK) (담당자)
title (NOT NULL)
description (NOT NULL)
service_category_id (FK) (NOT NULL)
status (REQUESTED, INTAKE, IN_PROGRESS, COMPLETED, CONFIRMED, REJECTED, ON_HOLD) (NOT NULL)
priority (CRITICAL, HIGH, MEDIUM, LOW) (NOT NULL)
...
#### 9. SRStatusHistory (SR 상태 변경 이력)
```
id (PK)
sr_id (FK) (NOT NULL)
previous_status
current_status (NOT NULL)
changed_by (FK) (NOT NULL)
change_reason
changed_at (NOT NULL)
```
...
user_id (FK) (NOT NULL)
sr_id (FK)
notification_type (SR_CREATED, SR_ASSIGNED, SR_COMPLETED, SR_REJECTED, etc.)
title
content
channel (EMAIL, MATTERMOST, BOTH)
...
description
module (USER, CLIENT, SR, REPORT, etc.)
action (READ, CREATE, UPDATE, DELETE, etc.)
created_at (NOT NULL)
```

#### 13. RolePermission (역할-권한 매핑)
```
id (PK)
role_id (FK) (NOT NULL)
...
│  ├─ PostgreSQL: 8GB 데이터베이스
│  ├─ Storage: 100GB 파일 저장
│  ├─ Bandwidth: 250GB/월
│  └─ Point-in-time Recovery (7일)
├─ Upstash Redis: Pay-as-you-go ($10~20/월)
│  └─ 100K commands/day 예상
├─ Resend: Pro tier ($20/10K emails)
│  └─ 10,000 emails/month
├─ Inngest: Pro tier ($30/월)
│  └─ 500K steps/month
└─ Sentry: Team ($26/월)
   └─ 에러 모니터링

예상 월 비용: $131~161
적합 규모: 사용자 1,000명, SR 10,000건/월
```

**대규모/엔터프라이즈 (월 $400+)**

```
├─ Vercel: Enterprise (커스텀 가격, ~$300/월)
│  └─ 무제한 대역폭, SLA 보장
├─ Supabase: Team/Enterprise ($599/월~)
│  ├─ PostgreSQL: 무제한 데이터베이스
│  ├─ Storage: 무제한 파일 저장
│  ├─ Bandwidth: 무제한
│  ├─ Point-in-time Recovery (30일)
│  ├─ 99.9% SLA
│  └─ 전용 서버 옵션
│  또는 자체 PostgreSQL 호스팅 (AWS RDS 등)
├─ AWS ElastiCache Redis ($50~100/월)
│  └─ cache.t3.medium
├─ SendGrid: Pro ($90~200/월)
│  └─ 100K+ emails/month
├─ Inngest: Enterprise (커스텀)
└─ Sentry: Business ($99/월)

예상 월 비용: $438~1,398+ (Supabase Team 기준)
적합 규모: 사용자 5,000명+, SR 50,000건/월
```
