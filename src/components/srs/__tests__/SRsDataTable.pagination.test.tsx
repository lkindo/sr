...
vi.mock('@/components/ui/pagination', () => ({
  ...
  PaginationPrevious: ({ onClick, 'aria-disabled': disabled }: any) => (
    <button onClick={onClick} disabled={disabled} aria-label="Go to previous page">Previous</button>
  ),
  PaginationNext: ({ onClick, 'aria-disabled': disabled }: any) => (
    <button onClick={onClick} disabled={disabled} aria-label="Go to next page">Next</button>
  ),
...
vi.mock('@/components/ui', () => ({
    ...
    Select: ({ children, onValueChange, value }: any) => <div data-value={value} onClick={() => onValueChange && onValueChange('10')}>{children}</div>,
    SelectTrigger: ({ children, className, 'aria-label': ariaLabel }: any) => <div className={className} aria-label={ariaLabel}>{children}</div>,
