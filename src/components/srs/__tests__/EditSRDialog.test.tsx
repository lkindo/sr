...
describe('EditSRDialog Component', () => {
    ...
    const mockSR = {
        ...
        attachments: [
            { id: 'att-1', fileName: 'test.pdf', fileSize: 1024, fileUrl: 'http://example.com/test.pdf', createdAt: new Date().toISOString() }
        ]
    };
    ...
    it('handles attachment deletion', async () => {
        ...
        if (deleteBtn) {
            ...
            await waitFor(() => {
                expect(global.fetch).toHaveBeenCalledWith('/api/attachments/att-1', expect.objectContaining({ method: 'DELETE' }));
                expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ description: '파일이 삭제되었습니다.' }));
            });
        }
    });
