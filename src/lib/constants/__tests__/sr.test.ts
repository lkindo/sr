 ...
 * 사본들은 오히려 enum 으로 타입이 걸려 있었고 **정본만 무방비**였다.
 *
...
describe('SR 상태 라벨 정본', () => {
  ...
  it('ON_HOLD 는 보류다', () => {
    // 2026-08-09 확정. 정본만 '대기' 이고 사본들은 이미 '보류' 여서, 같은 상태가
    // 화면에 따라 두 이름으로 보였다 — 목록에서 '보류' 를 보고 상세로 들어가면 '대기' 가 떴다.
    expect(statusLabels.ON_HOLD).toBe('보류');
  });

  it('REJECTED 는 거절이다', () => {
    // 2026-08-10 확정. '거부' 를 버린 이유는 도메인이 아니라 어휘 충돌이다 —
    // 이 저장소에서 '거부' 는 인가·검증 거부(403/400/429)를 뜻하는 지배 어휘이고,
    // 알림 설정의 '권한 거부됨' 과 글자까지 같아 로그·스크린샷에서 구분이 안 됐다.
    // 사람이 읽는 문구(버튼·다이얼로그·API 오류·메일·문서)는 이미 전부 '거절' 이었다.
