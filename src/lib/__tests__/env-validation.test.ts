...
import { validateEnv, ENV_VARIABLES, EnvValidationError, printEnvSummary, validateAndPrintEnv } from '../env-validation';
...
describe('EnvValidation', () => {
    ...
    describe('EnvValidationError', () => {
        ...
        it('should format error message correctly with invalid variables', () => {
            const error = new EnvValidationError(
                [],
                [{ variable: { name: 'TEST_VAR', required: true, description: 'Test', category: 'auth' }, error: 'Invalid format' }]
            );
        ...
        it('should format error message with both missing and invalid variables', () => {
            const error = new EnvValidationError(
                ...
                [{ variable: { name: 'INVALID_VAR', required: true, description: 'Invalid', category: 'auth' }, error: 'Bad value' }]
            );
            expect(error.message).toContain('누락된 필수 환경 변수');
            expect(error.message).toContain('유효하지 않은 환경 변수');
        });
