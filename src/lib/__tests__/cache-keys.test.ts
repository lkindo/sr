...
describe('cache-keys', () => {
  ...
  it('SR 목록 키를 생성해야 한다', () => {
    expect(srListKey({ status: 'OPEN', clientId: 'cli', priority: 'HIGH' })).toBe(
      'sr:list:OPEN:cli:HIGH'
    );
  ...
  it('대시보드 키는 기본값을 가진다', () => {
    ...
    expect(dashboardStatsKey({ userId: 'user', roleScope: 'admin' })).toBe(
      'dashboard:stats:user:admin'
    );
  });

  it('내 요청 키를 생성해야 한다', () => {
    expect(myRequestsKey('user', null, undefined)).toBe('srs:my-requests:user:all:createdAt');
    expect(myRequestsKey('user', 'OPEN', 'updatedAt')).toBe('srs:my-requests:user:OPEN:updatedAt');
  });
});
