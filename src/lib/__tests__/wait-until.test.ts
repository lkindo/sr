...
describe('wait-until', () => {
  describe('backgroundTask', () => {
    it('should log success when promise resolves', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('TestTask completed successfully')
      );
    });
    ...
    it('should log error when promise rejects', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('TestTask failed'), error);
    });
  });
  ...
  describe('backgroundTasks', () => {
    it('should handle multiple tasks', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('Task1 completed successfully')
      );
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('Task2 completed successfully')
      );
    });
  });
});
