...
const emptyStringAndNullToUndefined = (val: unknown) => (val === "" || val === null ? undefined : val);
...
export const clientCreateSchema = z.object({
  ...
  contactEmail: z.preprocess(emptyStringToUndefined, z.string().email("유효한 이메일 주소를 입력해주세요.").optional()),
...
export const srUpdateSchema = z.object({
  title: z.preprocess(emptyStringToUndefined, z.string().min(5, "제목은 최소 5자 이상이어야 합니다.").optional()),
  description: z.preprocess(emptyStringToUndefined, z.string().min(10, "설명은 최소 10자 이상이어야 합니다.").optional()),
  ...
  priority: z.preprocess(emptyStringToUndefined, z.enum(["CRITICAL", "HIGH", "MEDIUM", "LOW"]).optional()),
  status: z.preprocess(emptyStringToUndefined, z.enum([
  ...
  satisfactionRating: z.preprocess((val) => (val === "" ? null : typeof val === "string" ? parseInt(val, 10) : val), z.number().min(1).max(5).optional().nullable()),
  additionalFeedback: z.preprocess(emptyStringToNull, z.string().optional().nullable()),
  actualPriority: z.preprocess(emptyStringToUndefined, z.enum(["CRITICAL", "HIGH", "MEDIUM", "LOW"]).optional()),
  estimatedHours: z.preprocess((val) => (val === "" ? null : typeof val === "string" ? parseFloat(val) : val), z.number().positive("예상 작업 시간은 0보다 커야 합니다").optional().nullable()),
  ...
  assigneeId: z.preprocess(emptyStringToUndefined, z.string().min(1, "담당자를 선택해주세요").optional()),
