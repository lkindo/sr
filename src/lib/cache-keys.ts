...
export function srListKey(filters: { status?: string | null; clientId?: string | null; priority?: string | null }) {
...
export function dashboardStatsKey(scope: { userId?: string; roleScope?: 'admin' | 'client' | 'engineer' | 'generic' } = {}) {
	const userId = scope.userId ?? 'global'
	const role = scope.roleScope ?? 'generic'
	return `${DASHBOARD_STATS_PREFIX}${userId}:${role}`
}

export function myRequestsKey(userId: string, status: string | null | undefined, sortBy: string | null | undefined) {
