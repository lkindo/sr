...
vi.mock('@/lib/prisma', () => {
    const mock = {
        ...
        sRStatusHistory: { deleteMany: vi.fn().mockResolvedValue({}), findMany: vi.fn().mockResolvedValue([]), count: vi.fn().mockResolvedValue(0) },
    };
    return { default: mock };
});
...
describe('SRService Mutation Tests', () => {
    ...
    const mockUser = {
        ...
        roles: ['USER'], permissions: [], clientIds: []
    };
    ...
    describe('createSR', () => {
        it('should call prisma.create with correct data', async () => {
            ...
            const data = {
                ...
                clientId: 'c-1', serviceCategoryId: 'cat-1',
        ...
        it('should throw if client is inactive', async () => {
            vi.mocked(ensureCanCreateSR).mockReturnValue(undefined);
            vi.mocked(prisma.client.findUnique).mockResolvedValue({ id: 'c-1', isActive: false, name: 'Inactive' } as any);

            const data = {
                title: 'Test Title', description: 'Long enough desc',
                clientId: 'c-1', serviceCategoryId: 'cat-1',
                requestedPriority: 'MEDIUM' as const
            };
    ...
    describe('getAllSRs', () => {
        it('should return list of SRs', async () => {
            const mockSRs = [{ id: 'sr-1', title: 'SR 1' }, { id: 'sr-2', title: 'SR 2' }];
            ...
            expect(prisma.sR.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 0, take: 10 }));
        });
    });
