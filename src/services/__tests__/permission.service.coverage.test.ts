...
describe('PermissionService Coverage', () => {
    ...
    describe('requireRole', () => {
        ...
        it('throws Error if user does not have role', async () => {
            ...
            await expect(permissionService.requireRole('u1', 'ADMIN')).rejects.toThrow('필요한 역할이 없습니다');
        });
    });
    ...
    describe('requirePermission', () => {
        ...
        it('throws ForbiddenError if user lacks permission', async () => {
            vi.mocked(prisma.user.findUnique).mockResolvedValue({
                id: 'u1', isActive: true, roles: []
            } as any);
            await expect(permissionService.requirePermission('u1', 'SR:CREATE')).rejects.toThrow(ForbiddenError);
        });
    });
