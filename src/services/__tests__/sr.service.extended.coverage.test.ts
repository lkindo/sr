...
vi.mock('@/services/push.service', () => ({
    pushService: { sendToUser: vi.fn().mockResolvedValue(undefined), sendToUsers: vi.fn().mockResolvedValue(undefined) }
}));
...
describe('SRService Extended Branches', () => {
    ...
    describe('updateSR Notifications', () => {
        it('skips emails if preferences are false', async () => {
            vi.mocked(prisma.sR.findUnique).mockResolvedValue({ id: 'sr-1', status: 'INTAKE', requesterId: 'req-1', assigneeId: null } as any);
    ...
    describe('updateSR Date and Type Branches', () => {
        it('updates fields to non-null and null, handles assignedToId', async () => {
            vi.mocked(prisma.sR.findUnique).mockResolvedValue({ id: 'sr-1', status: 'IN_PROGRESS' } as any);
            vi.mocked(ensureCanUpdateSR).mockReturnValue(undefined);
        ...
        it('adjusts due date based on priority even if intakeAt is null', async () => {
            vi.mocked(prisma.sR.findUnique).mockResolvedValue({ id: 'sr-1', status: 'IN_PROGRESS', actualPriority: 'MEDIUM', serviceCategoryId: 'sc-1', intakeAt: null } as any);
            vi.mocked(ensureCanUpdateSR).mockReturnValue(undefined);
            vi.mocked(prisma.serviceCategory.findUnique).mockResolvedValue({ id: 'sc-1', slaHours: 24 } as any);
