...
const createRedisClient = (store: RedisStore) => ({
	...
	async scan(_cursor: number, { match }: { match: string; count: number }): Promise<[number, string[]]> {
...
async function loadRedisCache() {
	...
	vi.unstubAllEnvs()
	vi.resetModules()
	;(globalThis as typeof globalThis & { __SR_TEST_REDIS__?: any }).__SR_TEST_REDIS__ = createRedisClient(store) as any
