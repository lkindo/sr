...
## 2024-03-12 - Missing IDOR Protection in Nested SR Resources
**Vulnerability:** API routes handling nested resources of SRs (`/api/attachments`, `/api/srs/[id]/attachments`, and `/api/srs/[id]/comments`) were retrieving the `SR` instance but failing to call authorization policies like `ensureCanReadSR` or `ensureCanUpdateSR` before allowing the operation.
**Learning:** Checking the existence of a resource (`if (!sr) throw NotFound`) is not the same as verifying permissions. Even when using the `withAuthAndRateLimit` wrapper, it only guarantees authentication, not authorization. Furthermore, to use policy checks like `ensureCanUpdateSR` effectively, the database query (`prisma.sR.findUnique`) must fetch the full SR object (or use `include` for associated entities like `requester` and `assignee`), rather than restricting it via `select`.
**Prevention:** Always follow up a `findUnique` query for a secured resource with the appropriate policy check (e.g., `ensureCanReadSR(session.user, resource)` or `ensureCanUpdateSR(session.user, resource)`) before performing any sensitive actions such as file upload or returning data.
